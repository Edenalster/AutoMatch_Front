...
const PortalUserDetail = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      try {
        ...
        const response = await fetch(`${backendUrl}/api/lichess/dashboard/users/${lichessId}`);
    ...
    const fetchTournamentData = async () => {
      ...
      try {
        ...
        const response = await fetch(`${backendUrl}/api/lichess/dashboard/users/${lichessId}/tournaments`);
        ...
        <Card className="bg-chess-dark/30 backdrop-blur-md border border-white/10">
          <CardContent className="pt-6 pb-6">
            <div className="flex flex-col items-center justify-center text-center p-8">
              <h2 className="text-xl font-semibold text-white mb-2">Error Loading User Data</h2>
              <p className="text-white/70 mb-4">{error || "The requested user could not be found."}</p>
  ...
  return (
    <div className="p-8">
            ...
            <div className="flex-1 space-y-4">
              <div>
                <h2 className="text-2xl font-bold text-white mb-2">
                  {displayName}
                  {user.title && (
                    <span className="ml-2 text-chess-gold text-lg">{user.title}</span>
                  )}
                </h2>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-white">{user.rating || 1500}</p>
              </div>
        ...
        <Card className="bg-chess-dark/30 backdrop-blur-md border border-white/10">
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              ...
              <User className={`h-8 w-8 ${user.status === "active" ? "text-green-400" : "text-slate-400"}`} />
            </div>
          </CardContent>
        </Card>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className={`text-lg font-bold ${
                  ...
                  {user.cheatingDetected ? `${user.cheatingCount} Detection${user.cheatingCount !== 1 ? 's' : ''}` : "Clean"}
                </p>
              </div>
              {user.cheatingDetected ? (
      ...
      {user.cheatingDetected && user.cheatingDetections && user.cheatingDetections.length > 0 && (
          ...
          <CardContent>
            <div className="space-y-3">
              {user.cheatingDetections.map((detection, index) => (
                <div key={index} className="border border-red-500/20 rounded-lg bg-red-500/5 overflow-hidden">
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-white font-semibold">Game ID: {detection.gameId}</p>
                          <p className="text-white/60 text-sm">
                            {new Date(detection.timestamp).toLocaleDateString()} at {new Date(detection.timestamp).toLocaleTimeString()}
                          </p>
                        </div>
                      </div>
                  ...
                  {expandedDetections.has(index) && (
                    <div className="px-4 pb-4 border-t border-red-500/20 bg-red-500/5">
                      <div className="pt-3">
                        <h4 className="text-white/80 font-medium mb-2">Analysis Details:</h4>
                        <p className="text-white/70 text-sm leading-relaxed">{detection.analysis}</p>
                      </div>
                    </div>
                  )}
                </div>
              ...
              <Table>
                <TableHeader>
                  <TableRow className="border-white/10 hover:bg-white/5">
                    ...
                    <TableHead className="text-center text-white/80">Players</TableHead>
                    <TableHead className="text-center text-white/80">Prize Pool</TableHead>
                    <TableHead className="text-center text-white/80">Result</TableHead>
                    <TableHead className="text-center text-white/80">Position</TableHead>
                    <TableHead className="text-center text-white/80">W/L</TableHead>
                    <TableHead className="text-center text-white/80">Status</TableHead>
                    <TableHead className="text-center text-white/80">Date</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                    ...
                    >
                      ...
                      <TableCell className="text-center">
                        <div className="text-white text-sm">
                          <span className="text-green-400">{tournament.matchesWon}</span>
                          /
                          <span className="text-red-400">{tournament.matchesLost}</span>
                      ...
                      <TableCell className="text-center">
                        <Badge className={
                          ...
                          {tournament.status === "active" && <Clock className="h-3 w-3 mr-1" />}
